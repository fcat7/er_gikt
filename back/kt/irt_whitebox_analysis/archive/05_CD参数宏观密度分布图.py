...
def main():
    ...
    c_max = 0.50
    d_max = 0.20
    guessing_prob_init = getattr(config, 'guessing_prob_init', 0.05)
    slipping_prob_init = getattr(config, 'slipping_prob_init', 0.02)
    ...
    c_real = c_max * sigmoid(c_weights + guess_offset)
    d_real = d_max * sigmoid(d_weights + slip_offset)
    ...
    out_path = os.path.join(out_dir, f"05_CD参数宏观密度分布图.png")
