...
def main():
    ...
    for t_name, count in type_counts.items():
        mapped = type_map.get(t_name, t_name)
        print(f" - {mapped:<15}: {count} 题 (占比 {count/total_used*100:.1f}%)")
    ...
    labels = [type_map.get(l, l) for l in type_counts.index.tolist()]
    ...
    for name, count in type_counts.items():
        ...
        table_data.append([mapped, f"{count}", f"{pct:.2f}%", f"{prior:.2f}"])
    ...
    timestamp = "" # 移除随机后缀，保持文件名整洁
    out_dir = os.path.dirname(os.path.abspath(__file__))
    
    out_path_png = os.path.join(out_dir, "03_数据集题型真实先验分布.png")
    out_path_pdf = os.path.join(out_dir, "03_数据集题型真实先验分布.pdf")
