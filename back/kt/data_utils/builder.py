...
class KTDataBuilder:
    ...
    def build_sequences(self, df, stride=100, save_prefix="train"):
        ...
        for user_id, group in grouped:
            ...
            col_map = {c: i for i, c in enumerate(group.columns)}
            # itertuples 确实快一些
            
            # 预取所需列的 index
            # idx_pid = col_map.get('problem_id') ... 
            # 直接用 itertuples(index=False) 配合 getattr
            ...
            # 修正最后一个窗口：确保不被遗漏
            if starts and starts[-1] + max_len < total_interactions:
                # 最后剩余的数据如果>0 就要保留
                pass  # range已处理
            ...
            for start in starts:
                ...
                # 过滤过短序列（Train模式下）
                is_train = 'train' in save_prefix
                if is_train and length < min_len and len(starts) > 1:
                    continue
                if length <= 0: 
                    continue
    ...
    def _deprecated_build_dataset(self, df, enable_window_aug=False, stride=100):
        """
        [已弃用] 旧版构建方法，保留以防万一
        """
        pass
