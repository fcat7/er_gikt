...
class KTDataSampler:
    ...
    def stratified_sample(self, df, ratio=0.1, min_seq_len=MIN_SEQ_LEN, user_col=StandardColumns.USER_ID, label_col=StandardColumns.LABEL):
        """
        ...
        # 1. 预过滤：只考虑交互数足够的学生
        # 如果 df 中没有 label 列，降级为仅 Len 分层
        if label_col not in df.columns:
            ic(f"Warning: '{label_col}' column missing. Fallback to Length-only stratification.")
            return self._stratified_sample_legacy(df, ratio, min_seq_len, user_col)
        ...
        # 4. 提取数据
        sampled_df = df[df[user_col].isin(sampled_user_ids)].copy()
        
        ic(f"采样完成: {len(df)} -> {len(sampled_df)} (Users: {len(valid_users)} -> {len(sampled_user_ids)})")
        return sampled_df

    def _stratified_sample_legacy(self, df, ratio=0.1, min_seq_len=MIN_SEQ_LEN, user_col=StandardColumns.USER_ID):
        """
        [旧版] 仅基于交互长度采样
        """
        # 1. 预过滤：只考虑交互数足够的学生
        ...
        user_interaction_counts = valid_df.groupby(user_col).size().reset_index(name='counts')
        ...
        for bucket in user_interaction_counts['bucket'].unique():
            bucket_users = user_interaction_counts[user_interaction_counts['bucket'] == bucket][user_col]
        ...
        # 4. 提取数据
        sampled_df = valid_df[valid_df[user_col].isin(sampled_users)]
