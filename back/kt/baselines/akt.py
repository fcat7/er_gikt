...
class AKT(nn.Module):
    def __init__(self, n_question, n_skill=None, d_model=256, n_blocks=1, dropout=0.05, d_ff=256, 
        ...
        # 1:1 复刻 PyKT AKT 的 Context-Aware Rasch Embedding
        # q_embed: 以知识点为键的概念嵌入 (concept embedding)
        self.c_embed = nn.Embedding(self.n_skill + 10, embed_l)
        # difficult_param: 以题目ID为键的 **标量** 难度参数（PyKT 原版是 1 维！）
        self.q_diff_embed = nn.Embedding(self.n_question + 10, 1)
        # q_embed_diff: 以知识点为键的概念变化量 (concept variation)
        ...
        # qa_embed_diff: 以回答(0/1)为键的交互变化量 (interaction variation)
    ...
    def base_emb(self, q_data, c_data, target):
        ...
        c_emb = self.c_embed(c_data)            # [bs, seq, d_model]
        q_diff = self.q_diff_embed(q_data)       # [bs, seq, 1]  ← 标量！
        c_var = self.c_var_embed(c_data)          # [bs, seq, d_model]

        # PyKT 原版公式: q_embed = c_ct + mu_q * d_ct
        ...
        r_var = self.r_var_embed(target)          # [bs, seq, d_model]
        # PyKT/pyedmine 原版公式: qa_embed = (c_ct + g_rt) + mu_q * (f_rt + d_ct)
    ...
    def forward(self, question, response, mask=None, interval_time=None, response_time=None, skill=None):
        ...
        d_output = self.model(q_embed_data, qa_embed_data, pid_embed_data, padding_mask=mask)

        # Shift q_embed_data to predict next question
        # d_output at t contains information up to t
        # we want to predict response at t+1 using q_embed_data at t+1
        # so we concatenate d_output[:, :-1] with q_embed_data[:, 1:]
        concat_q = torch.cat([d_output[:, :-1, :], q_embed_data[:, 1:, :]], dim=-1)
        output = self.out(concat_q).squeeze(-1)
        # Return logits to be compatible with BCEWithLogitsLoss
        return output
...
class Architecture(nn.Module):
    ...
    def forward(self, q_embed_data, qa_embed_data, pid_embed_data, padding_mask=None):
        ...
        for block in self.blocks_1:  # encode qas, 对0～t-1时刻前的qa信息进行编码
            y = block(causal_k=1, query=y, key=y, values=y, pdiff=pid_embed_data, padding_mask=padding_mask) # yt^
        flag_first = True
        for block in self.blocks_2:
            if flag_first:  # peek current question
                x = block(causal_k=1, query=x, key=x,
                          values=x, apply_pos=False, pdiff=pid_embed_data, padding_mask=padding_mask) # False: 没有FFN, 第一层只有self attention, 对应于xt^
                flag_first = False
            else:  # dont peek current response
                x = block(causal_k=0, query=x, key=x, values=y, apply_pos=True, pdiff=pid_embed_data, padding_mask=padding_mask) # True: +FFN+残差+laynorm 非第一层与0~t-1的的q的attention, 对应图中Knowledge Retriever
                # causal_k=0，不能看到当前的response, 在Knowledge Retrever的value全为0，因此，实现了第一题只有question信息，无qa信息的目的
                # print(x[0,0,:])
                flag_first = True
        return x
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, causal_k, query, key, values, apply_pos=True, pdiff=None, padding_mask=None):
        """
        Input:
            block : object of type BasicBlock(nn.Module). It contains masked_attn_head objects which is of type MultiHeadAttention(nn.Module).
            causal_k : 0 means, it can peek only past values. 1 means, block can peek only current and pas values
        ...
        nopeek_mask = np.triu(
            np.ones((1, 1, seqlen, seqlen)), k=causal_k).astype('uint8')
        src_mask = (torch.from_numpy(nopeek_mask) == 0).to(query.device)
        
        # 合并 Padding Mask
        if padding_mask is not None:
            # padding_mask shape: [batch_size, seq_len]
            # 广播为: [batch_size, 1, seq_len, seq_len] 并与因果掩码相乘
            # src_mask 是布尔掩码，True 表示允许关注，False 表示屏蔽
            p_mask = padding_mask.view(batch_size, 1, 1, seqlen).bool()
            src_mask = src_mask & p_mask

        if causal_k == 0:  # If 0, zero-padding is needed.
